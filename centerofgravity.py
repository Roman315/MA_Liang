...
def center(df_error_w, time, adaptive_threshold, hole_search_filter_mat_size):
    
    #create empty list outside of loop to avoid assignment error in case of n = 0
    circles = []
    text_dists = []
    df_holes_num, ns, x1_centers, x2_centers, radius, depth = find_holes(df_error_w, hole_search_filter_mat_size, adaptive_threshold)
    
    # create output dataframe
    df_output = np.zeros((ns + 1, 5))
    ...
    for i in np.arange(0, len(x1_centers)):
        df_output.at[i, 'Lochnummer'] = i
        df_output.at[i, 'Durchmesser'] = radius[i] * 2 * 0.27
        df_output.at[i, 'Tiefe'] = depth[i]
        df_output.at[i, 'Std_Durchmesser'] = radius[i] * 2 * 0.27
        df_output.at[i, 'Std_Tiefe'] = depth[i]
        #df_output.at[n, 'Tiefe2'] = aver_depth_pro
        circle = plt.Circle((x2_centers[i], x1_centers[i]), radius[i], color='r', fill=False)
        circles.append(circle)
    # circles = plt.Circle((x1_centers, x2_centers), radius, color='r', fill=False)
    # prepare plot
    #ax5.add_artist(circle) # add circle to plot
    text_dists = [np.sqrt(2)/2*x for x in radius] # distance for position of number in plot
    #ax5.text(x1_center + text_dist , x2_center - text_dist , n, color="red") # add hole number to plot
    # df_depth_with_num = pd.DataFrame(df_output['Lochnummer'])
    return df_output, df_holes_num, circles, text_dists, x1_centers, x2_centers, ns

def find_holes(df_to_process, filter_window_hole_search, adaptive_threshold):

    imprints = (ndimage.median_filter(df_to_process, filter_window_hole_search)[:,:] < adaptive_threshold)
    ...
    num_holes = int(0)
    while num_holes < len(x_all_points):
        i = -1
    ...
    x1_centers, x2_centers, radius = get_diameter(x_all_points, y_all_points, x_boundary, y_boundary, df_groups)
    # print('x_all', x_all_points)
    depth = get_depth(df_to_process.to_numpy(), x_all_points, y_all_points)    
    return df_groups, detected_num_holes, x1_centers, x2_centers, radius, depth
...
def get_depth(df_to_process, x_all_points, y_all_points):
    ...
    for j in range(0, len(x_all_points)):
        ...
        for j_1 in range(0, len(x_all_points[j])):
            # print('j_1', j_1)
            height.append(abs(df_to_process[x_all_points[j][j_1], y_all_points[j][j_1]]))    # using absolute value for easy comparison

        height.sort()
        depth.append(sum(height[-2:]) / len(height[-2:]))
    return depth